...
job* findJobByIndex(job* job_list, int idx){
    ...
    if(tmp == NULL) {
        printf("could not find a job with id %d\n", idx);		
    }
    return tmp;
}
...
void updateJobList(job **job_list, int remove_done_jobs){
    ...
    while (tmp != NULL){
            ...
            if (tmp->status == 1) { /* running */
                    waitpid(tmp->pgid, &tmp->status, WNOHANG); /* after this tmp will be done */
            }
            tmp = tmp -> next;
    }
